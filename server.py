...
Serves `benchmark-main.html` at `/` and `benchmark-detail.html` at `/detail`,
plus an index page listing both.
"""
...
def _serve(handler: BaseHTTPRequestHandler, path: str) -> None:
    if path not in REPORTS:
        handler.send_error(HTTPStatus.NOT_FOUND, f"No report at {path}")
        return
    title, filename = REPORTS[path]
    body = (SCRIPT_DIR / filename).read_bytes()
    handler.send_response(HTTPStatus.OK)
    handler.send_header("Content-Type", "text/html; charset=utf-8")
    handler.send_header("Content-Length", str(len(body)))
    handler.end_headers()
    handler.wfile.write(body)
    print(f"[{handler.address_string()}] {path} -> {title} ({len(body)} bytes)")
...
class ReportHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:  # noqa: N802 (BaseHTTPRequestHandler API)
        ...
        if path == "/":
            links = "".join(
                f'<li><a href="{route}">{title}</a></li>'
                for route, (title, _) in REPORTS.items()
            )
    ...
    def log_message(self, *_: object) -> None:  # silence default logging
        pass
